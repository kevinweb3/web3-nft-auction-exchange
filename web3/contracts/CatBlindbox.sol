...
contract CatBlindbox is ERC721 {
    event ResultOfNewBlindboxCat(
        address sender,
        string catName,
        bytes32 requestId
    );
    ...
    constructor(address randomOracle) ERC721("CatBlindBoxSeries", "Cat") {
    ...
    function generateBlindBoxCat(bytes32 requestId) public {
        require(catSpeciesPool.length > 0, "There are no blind box cats!");
        require(
            oracle.checkIdFulfilled(requestId) == false,
            " oracle query has not been fulfilled!"
        );
